...
public class controller extends HttpServlet {
		...
		if(request.getParameter("startDate") == null || request.getParameter("startDate").equals("")) {
			...
			if(checkValidDates(request.getParameter("startDate")) && checkValidDates(request.getParameter("endDate"))) {
				try {         
					...
					userSession.setAttribute("errorMessage", e.getMessage());
				}
			}else {
				message = "Invalid date/s entered.";
			}
		}
		userSession.setAttribute("errorMessage", message);
		redirect(request, response,"/ITViews/ITMainPage.jsp");
	}
	...
	/*
	 * Set session lists stafflist and statuslist to populate drop down lists
	 * */
	...
	/*
	 * Used with included jsp page updateIssueForm
	 * Calls data access methods to update the issue's status or currently assigned ITStaffID, in the database
	 * Update's the session objects: errorMessage (Displays a success msg) and currentIssue (so the page displays the updated info once redirected)
	 * */
	private void updateIssue(UserDA dataAccess, HttpSession userSession, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			userSession.setAttribute("errorMessage", "Success! Issue has been updated.");
	...
	/*
	 * Sets myIssues, pendingIssues, maintenanaceDates (only if null), call populateDropDown to populate statusList and staffList
	 * */
