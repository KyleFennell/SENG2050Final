	...
	<script>
	  	  /*JQuery Date pickers located in Notification.jsp include file */
		  $(function() {
	...
	<c:choose>
		<c:when test="${sessionScope.maintenanceDates != null}">
			<table>
	...
	<c:choose>
		<c:when test="${userLoggedIn.isStaff()}">
			...
			<c:choose>
				<c:when test="${sessionScope.errorMessage == 'Maintenance successfully added.'}">
